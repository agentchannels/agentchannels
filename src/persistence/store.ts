...
import { assertTransition } from "../core/session-state.js";
  ...
  const inspect = (candidate: unknown): void => {
    ...
    for (const [key, child] of Object.entries(candidate)) {
      if (forbidden.test(key))
        throw new Error(`Sensitive metadata field ${key} cannot be persisted`);
      inspect(child);
    }
  };
  inspect(value);
...
export class AmbiguousAgentError extends Error {
  constructor(
    ...
    super(`More than one Agent is registered for CWD ${cwd}`);
  }
}
...
export class Persistence {
  ...
  deleteAgent(agentId: string): boolean {
    ...
    if (bindingCount.count > 0)
      throw new Error(`Agent ${agentId} still has Bindings; remove them first`);
    return (
  ...
  completeBindingSetup(
    ...
    return this.transaction(() => {
      ...
      if (setup === undefined)
        throw new Error(`Binding setup ${setupId} not found`);
  ...
  deleteBinding(bindingId: string): boolean {
    ...
    if (deliveryCount.count > 0)
      throw new Error(`Binding ${bindingId} still has pending deliveries`);
    return (
  ...
  retireSession(sessionId: string, at = new Date()): boolean {
    ...
    if (["queued", "running", "waiting"].includes(session.status)) {
      throw new Error(`Active Session ${sessionId} cannot be retired`);
    }
    if (
      session.retentionExpiresAt === null ||
      session.retentionExpiresAt > iso(at)
    ) {
      throw new Error(`Session ${sessionId} is still retained`);
    }
    return (
  ...
  updatePendingInteractionResponse(
    ...
    if (changed !== 1)
      throw new Error(`Pending Interaction ${interactionId} was not found`);
    return required(
  ...
  resolveInteraction(
    ...
    if (existing.status !== "pending")
      throw new Error(`Interaction ${interactionId} is already resolved`);
    this.db
      .prepare(
  ...
  markFollowUpDelivered(followupId: string, at = new Date()): FollowUp {
    ...
    if (!row) throw new Error(`Follow-up ${followupId} not found`);
  ...
  hasIngress(bindingId: string, eventId: string): boolean {
    return (
      this.db
        .prepare(
          "SELECT 1 FROM ingress_events WHERE binding_id=? AND event_id=?",
        )
        .get(bindingId, eventId) !== undefined
    );
  }
}

export { migrate } from "./schema.js";
export type Store = Persistence;
