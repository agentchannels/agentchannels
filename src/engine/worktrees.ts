...
export class WorktreeCreationError extends AgentChannelsError {
  constructor(message: string, cause: unknown) {
    super("INTERNAL_ERROR", message, ["Rerun with --debug for details."], {
      cause,
    });
...
export class WorktreeManager {
  ...
  async create(sessionId: string): Promise<Worktree> {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(sessionId)) {
      throw internalError("Session ID is not safe for a worktree path.");
    }
    ...
    if (!repositoryRoot)
      throw new AgentChannelsError(
        "MISSING_GIT_HEAD",
        "The configured directory is not a Git repository.",
        ["Point the Agent at a Git repository and rerun."],
      );
    ...
    if (!baseCommit)
      throw new AgentChannelsError(
        "MISSING_GIT_HEAD",
        "The repository has no current HEAD.",
        ["Create the repository\u2019s first commit, then retry."],
      );
    ...
    if (!worktreePath.startsWith(`${worktreeRoot}${path.sep}`)) {
      throw internalError(
        "Worktree path escapes the owned worktree directory.",
      );
    }
  ...
  async remove(
    ...
    if (!worktreePath.startsWith(`${worktreeRoot}${path.sep}`))
      throw invalidState(
        `Worktree is not owned by AgentChannels: ${worktreePath}`,
        ["Remove the worktree manually if it is no longer needed."],
      );
