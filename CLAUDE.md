...
│  commander program: ach serve | init slack | deploy railway │
└──────────┬──────────────────────────────────────────────────┘
           │
┌──────────▼──────────────────────────────────────────────────┐
...
### `ach init slack` — Full stack setup flow

All in `src/channels/slack/init.ts`. The wizard runs five phases in order before writing anything to `.env`:

1. **API Key** — Validates `ANTHROPIC_API_KEY` against `client.beta.agents.list()` as a health check; fails fast so Slack setup never starts with a broken key
2. **Agent** — Detects existing `CLAUDE_AGENT_ID` in `.env` (validates, warns if stale), lists agents via `client.beta.agents.list()`, or creates a new one via `client.beta.agents.create()`
3. **Environment** — Same detect/validate/list/create pattern using `client.beta.environments`; environments are independent top-level resources (not nested under agents)
4. **Vaults** — Optional: user enters comma-separated vault IDs (stored as `CLAUDE_VAULT_IDS`); vault *creation* is out of scope — select only
5. **Slack** — Three setup paths:
...
All five phases complete before `.env` is written. On success, writes `ANTHROPIC_API_KEY`, `CLAUDE_AGENT_ID`, `CLAUDE_ENVIRONMENT_ID`, `CLAUDE_VAULT_IDS` (if set), `SLACK_BOT_TOKEN`, `SLACK_APP_TOKEN`, and `SLACK_SIGNING_SECRET` in a single atomic write.
...
- **No .env in repo** — Generated by `ach init slack` (agent, environment, and Slack credentials all written in one flow)
